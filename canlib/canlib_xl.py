...
class can_api(object):
    ...
    def VISEappInit(self):
        err, hw_type, hw_index, hw_channel = self.driver.get_appl_config(appname='VISEapp', channel=0,
                                                                         bustype=XL_BUS_TYPE_CAN)
        self.mask = self.driver.get_channel_mask(hwtype=hw_type.value, hwindex=hw_index.value,
                                                 hwchannel=hw_channel.value)

        ok, self.phandle, self.pmask = self.driver.open_port(user_name='VISEapp', access_mask=self.mask,
                                                             bus_type=XL_BUS_TYPE_CAN)
        ok = self.driver.activate_channel(port_handle=self.phandle)
    ...
    def get_msg(self):
        ...
        if ok:
            ...
            rec_string = event_list
            print()
        return rec_string

    def get_msg_no_err(self):
        event_count = ctypes.c_uint(10)

        event_list = XLevent(10)
        ok = self.driver.receive(self.phandle, event_count, event_list)
        if ok:
            rec_string = self.driver.get_error_string(ok)
        else:
    ...
    def get_msgs_from_event_list(self, event_list):
        msg = event_list.tagData.msg
        msg_data = msg.data
        data = []
        data.append(msg_data[0])
        data.append(msg_data[1])
        data.append(msg_data[2])
        data.append(msg_data[3])
        data.append(msg_data[4])
        data.append(msg_data[5])
        data.append(msg_data[6])
        data.append(msg_data[7])
        print(str(data))
